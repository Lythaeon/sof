...
use sof_tx::{
    DirectSubmitConfig, RpcSubmitConfig, SignedTx, SubmitMode, SubmitTransportError, TxBuilder,
    TxSubmitClient,
    providers::{LeaderTarget, StaticLeaderProvider, StaticRecentBlockhashProvider},
    routing::RoutingPolicy,
    submit::{DirectSubmitTransport, RpcSubmitTransport},
};
...
fuzz_target!(|bytes: &[u8]| {
    ...
    let rpc = Arc::new(FuzzRpcTransport {
        should_fail: bytes.get(3).copied().unwrap_or(0) & 1 == 1,
        calls: AtomicU64::new(0),
    });
    ...
    match result {
        ...
        Err(error) => match error {
            ...
            | sof_tx::SubmitError::Direct { .. }
            | sof_tx::SubmitError::NoDirectTargets => {}
